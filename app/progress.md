...
- Closed Phase 5 on isolated Dokku staging at candidate `59f271c`: API and web serve over verified TLS, Rust authenticated/guest smoke passed, all three remote Playwright journeys passed, and staging run `staging-1789980490` returned zero missing or duplicate activation events. Production remained untouched.
- Staging caught and closed one deployment-only issue: Nginx eagerly resolved the Compose-only `server` hostname and restarted on standalone Dokku. The proxy now resolves that alias lazily through Docker DNS; a standalone image boot/health check and the Dokku deployment both pass.
- Remaining milestone checkpoint: the explicit human taste review of the complete generated guest-name cross product (`02-06`). Quiet-beta participant admission and the 50-host/50-invite cohort have not started.
