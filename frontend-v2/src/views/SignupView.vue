<template>
  <section class="auth-card">
    ...
    <form @submit.prevent="handleSubmit">
      ...
      <button class="primary" :disabled="auth.loading">{{ auth.loading ? 'Creating…' : 'Sign up' }}</button>
      <p class="helper" v-if="errorMessage">{{ errorMessage }}</p>
    </form>
    <p class="switcher">
...
async function handleSubmit() {
  ...
  try {
    ...
    errorMessage.value = auth.errorMessage ?? 'Failed to sign up';
  }
}
</script>
